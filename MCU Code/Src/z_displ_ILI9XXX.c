...
void Displ_Test()
{
	Displ_Select();
	    ...
	    uint8_t data_col[4] = {0x00, 0x00, 0x01, 0x3F};  // 0 to 319
	    Displ_WriteData(data_col, 4, 0);

	    Displ_WriteCommand(0x2B);  // Page Address Set
	    uint8_t data_page[4] = {0x00, 0x00, 0x01, 0xDF};  // 0 to 479
	    Displ_WriteData(data_page, 4, 0);

	    Displ_WriteCommand(0x2C);  // Memory Write

	    // Send colored stripes
	    for (int y = 0; y < 480; y++) {
	        uint16_t color;
	        if (y < 160)
	            color = 0xF800; // RED
	        ...
	        for (int x = 0; x < 320; x++) {
	            ...
	            Displ_WriteData(pixel_data, 2, 0);
	        }
	    }
