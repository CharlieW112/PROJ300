...
#define TOUCH_PRESCALER SPI_BAUDRATEPRESCALER_16	//prescaler assigned to touch device SPI port
...
#define BKLIT_TIMER 				TIM8			//timer used (PWMming DISPL_LED pin)
#define BKLIT_T 					htim8			//timer used
 ...
 **********************************************************/
#define DISPL_BAUDRATE_BITS   (5U << SPI_CFG1_MBR_Pos)   // for 64 prescaler
#define TOUCH_BAUDRATE_BITS   (7U << SPI_CFG1_MBR_Pos)   // for 256 prescaler

#define SET_DISPL_SPI_BAUDRATE \
    do { \
        DISPL_SPI->CR1 &= ~SPI_CR1_SPE; \
        DISPL_SPI->CFG1 = (DISPL_SPI->CFG1 & ~SPI_CFG1_MBR_Msk) | DISPL_BAUDRATE_BITS; \
        DISPL_SPI->CR1 |= SPI_CR1_SPE; \
    } while (0)

#define SET_TOUCH_SPI_BAUDRATE \
    do { \
        TOUCH_SPI->CR1 &= ~SPI_CR1_SPE; \
        TOUCH_SPI->CFG1 = (TOUCH_SPI->CFG1 & ~SPI_CFG1_MBR_Msk) | TOUCH_BAUDRATE_BITS; \
        TOUCH_SPI->CR1 |= SPI_CR1_SPE; \
    } while (0)
/**********************************************************/
